...
class DNMPs(nn.Module):
    def __init__(self,
        ...
        if embedding is None:
            ...
            self.mesh_embeddings = nn.Parameter(embedding, requires_grad=True)
    ...
    # hb: helper function to get pt from pixel
    def get_pt_from_pixel(pixel, depth, camera):
        '''
            pixel: [N,2]
            depth: [N]
            camera: Camera
...
class DNMPScene(nn.Module):
    def __init__(self,
        ...
        self.device = device
        self.decoder_fn = decoder_fn
        if predefined_ckpts is not None:
            decoder_fn_params = decoder_fn.state_dict()
            # load predefined_ckpts to self.decoder_fn
            for key, val in decoder_fn_params.items():
                decoder_fn_params[key] = predefined_ckpts[name+'.'+decoder_fn+'.'+key]
            self.decoder_fn.load_state_dict(decoder_fn_params)
            self.vertex_embeddings = nn.Parameter(predefined_ckpts[name+'.vertex_embeddings'], requires_grad=True)
    ...
    # hb: helper function to add mesh
    def add_mesh(self, voxel_centers, mesh_embeddings, voxel_size):
        print('Start to grow meshes')
        print(f'Original sphere number: {self.verts.shape[0]}')      
        print(f'Added sphere number: {voxel_centers.shape[0]}')      
        ...
        meshes = Meshes(verts.detach(), faces.detach())
        self.meshes = join_meshes_as_scene([self.meshes, meshes])
        self.verts = torch.cat([self.verts, verts], dim=0)
        self.faces = torch.cat([self.faces, faces], dim=0)
        print(f'New Sphere number: {self.verts.shape[0]}')
        
        verts_packed = meshes.verts_packed()
        pos_encoder = PositionEncoding(3, N_freqs=3)
        initial_verts_embedding = pos_encoder(verts_packed)
        self.vertex_embeddings = nn.Parameter(torch.cat([self.vertex_embeddings, initial_verts_embedding.float().to(self.device)], dim=0), requires_grad=True)
        
        self.num_verts_per_mesh = verts.shape[1]
        self.num_meshes += voxel_centers.shape[0]
        
    # hb: helper function to quantize the 3D points into voxels cited from models/renderer.py
