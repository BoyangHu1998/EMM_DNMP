...
class TrainerGeometry:
    def __init__(self, config): 
        ...
        if self.config.center_point_fn is not None:
            center_point = np.load(self.config.center_point_fn) # CHECK center point fn  TODO: CHECK
        ...
        if not os.path.exists(os.path.join(snapshot_dir, 'models')):
        ...
        self.optimizer = Adam([{'params': self.render.parameters()}], 
                               ...
                               betas=(0.9, 0.999),
                               weight_decay=0.)
        # (1) delete preivous gradient and parameter (2) reintialize + lr record 
    ...
    def train(self):
        ...
        while flag:
            for idx, sample in enumerate(self.train_loader):
                # self.save_ckpt(f'ckpt_{self.global_step}')  # for debug 
                ...
                rgb = sample['rgb'].squeeze() # Returns a tensor with all specified dimensions of input of size 1 removed. https://pytorch.org/docs/stable/generated/torch.squeeze.html
                ...
                pose_ = np.eye(4)
                pose_[:3, :4] = pose
                pose = np.matmul(self.transform_M, pose_) # TODO: check is if it is only for kitti!
                ...
                rays_o, rays_d = get_rays(intrinsic, pose, img_hw) # rays_o: (H*W, 3), rays_d: (H*W, 3) 
                rays_o = torch.from_numpy(rays_o).float().to(self.device) # rays_o: (H*W, 3) to numpy array as float
                rays_d = torch.from_numpy(rays_d).float().to(self.device) # rays_d: (H*W, 3)
                rgb = rgb.to(self.device) # rgb: (H, W, 3) image
                depth = depth.to(self.device) # depth: (H, W) depth image

                camera = get_camera(intrinsic, pose, img_hw, self.device) # camera: (H*W, 3, 3) # uncheck
                rays_o_, rays_d_, rgb_, pix_coords_idx = sample_rays(rays_o, rays_d, rgb, self.config.num_rays) # sample_rays 
                pix_coords_idx = torch.from_numpy(pix_coords_idx).long().to(self.device) # pix_coords_idx: (num_rays, 2)    

                # for debug: save rays_o_, rays_d_ into file
                # np.save('rays_o_.npy', rays_o_.cpu().numpy())
                # np.save('rays_d_.npy', rays_d_.cpu().numpy())        
                
                ret_dict = self.render.render_depth_normal(rays_o_, rays_d_, camera, pix_coords_idx, self.decoder_fn, img_hw) # render_depth_normal The function is used to render the depth and normal map of the mesh

                depth_ = depth.view(-1)[pix_coords_idx] # depth_: (num_rays)
                depth_loss = (torch.abs(ret_dict['depth'] - depth_)) # depth_loss: (num_rays)

                valid_idx_mesh = (ret_dict['depth'] > 0) # valid_idx_mesh: (num_rays)
                valid_idx_gt = (depth_ > 0) & (depth_ < self.config.valid_depth_thresh) # valid_idx_gt: (num_rays)

                depth_valid_idx = valid_idx_mesh & valid_idx_gt 
                
                if self.config.use_photo_check:
                    gt_depth_check = losses.photometric_loss( 
                        pred_rgb=rgb_, # pred_rgb: (num_rays, 3)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            # sample = self.val_iter.next() https://stackoverflow.com/a/75048982
    ...
    def save_ckpt(self, filename='checkpoint'):
        ...
        state = {
            ...
            'decoder_state_dict': self.decoder_fn.state_dict(),
        }
