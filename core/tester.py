...
class Tester:
    def __init__(self, config):
        ...
        for pretrained_geo in self.config.pretrained_geo_list:
            # pretrained_geo = "training_outputs/chpt/geo-dtu-0.001-test/ckpt_10000.pth"
            ...
            # voxel_centers = (voxel_centers + voxel_centers_offset).to(self.device)  # hb: remove offset the voxel centers 
        ...
        # # hb: 
        # voxel_size = [float(v) / self.config.scene_scale for v in self.config.voxel_size_list]
        # # Load pretrained render        
        # render_state_dict_after_growing = torch.load(self.config.pretrained_render)
        # self.render = render_state_dict_after_growing
    ...
    def test(self):
        self.render.eval()
        # self.mipnerf.eval() # Remove mipnerf 
        ...
        with torch.no_grad():
            ...
            for idx, sample in tqdm(enumerate(self.test_loader)):
                ...
                # comment out for testing the background colour
